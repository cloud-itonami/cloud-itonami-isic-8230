(ns eventops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (EventOperationsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the EventOperationsGovernor
  ...
  human event-operations coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [eventops.advisor :as advisor]
            [eventops.governor :as governor]
...
(defn build
  ...
  `eventops.store/Store`).
  opts:
    :advisor      -- an `eventops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; EventOperationsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; EventOperationsGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
