(ns eventops.phase
  "Phase 0->3 staged rollout for the ISIC-8230 'Organization of
  conventions and trade shows' operations-coordination actor.

    Phase 0  read-only              -- no writes, still governor-gated.
    Phase 1  assisted-logging       -- event-record logging allowed,
                                       every write needs human approval.
    Phase 2  assisted-coordination  -- adds venue-operation scheduling
                                       and vendor-order proposals, still
                                       approval-gated.
    Phase 3  supervised auto        -- governor-clean, high-confidence
                                       `:log-event-record`/
                                       `:schedule-venue-operation`/
                                       `:coordinate-vendor-order` may
                                       auto-commit. `:flag-safety-concern`
                                       NEVER auto-commits, at any phase,
                                       and a high-cost `:coordinate-
                                       vendor-order` still escalates even
  ...
  rollout milestone still to come. Flagging a crowd-safety/venue-
  occupancy-limit concern always needs a human to actually look at it.
  `eventops.governor`'s own `always-escalate-ops` enforces the same
  invariant independently -- two layers, not one, agree on this."
  (:require [eventops.governor :as governor]))
...
(def phases
  ...
  {0 {:label "read-only"              :writes #{}                                                              :auto #{}}
   1 {:label "assisted-logging"       :writes #{:log-event-record}                                             :auto #{}}
   2 {:label "assisted-coordination"  :writes #{:log-event-record :schedule-venue-operation
                                                 :coordinate-vendor-order}                                     :auto #{}}
   3 {:label "supervised-auto"        :writes write-ops
      :auto #{:log-event-record :schedule-venue-operation :coordinate-vendor-order}}})
...
(defn gate
  ...
  - `:flag-safety-concern` is never auto-eligible at any phase, so it
    ...
    governor doesn't). A high-cost `:coordinate-vendor-order` never
...
(defn verdict->disposition
  "Map an EventOperationsGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
